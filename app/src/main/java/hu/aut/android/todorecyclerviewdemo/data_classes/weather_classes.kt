package hu.aut.android.todorecyclerviewdemo.data_classes

data class Base(val coord: Coord?, val weather: List<Weather>?, val base: String?, val main: Main?, val visibility: Number?, val wind: Wind?, val clouds: Clouds?, val dt: Number?, val sys: Sys?, val id: Number?, val name: String?, val cod: Number?)
